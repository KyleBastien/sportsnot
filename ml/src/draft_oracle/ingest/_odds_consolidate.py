...
def consolidate_odds(
    ...
    from draft_oracle.ingest.odds import _ODDS_COLUMNS, XVAL_DELTA_THRESHOLD
    ...
    for anchor in _consolidation_order(records):
        if anchor["_used"] or _has_missing_team_ids(anchor):
            continue
        members = _claim_cluster_members(anchor, buckets)
        best = _best_cluster_row(anchor, members, keep)
        _blank_if_archive_unmatched(best, local_game_dates, local_game_types, stats)
...
def _blank_if_archive_unmatched(
    ...
    if not (archive_unmatched or reversed_only) or not bool(row["covered"]):
        return False
