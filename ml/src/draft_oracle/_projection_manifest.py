...
def _cli_flags(config: Any) -> dict[str, Any]:
    return {
...
def _count_summary(outputs: Any) -> dict[str, int]:
    return {
...
def _ir_stash_summary(outputs: Any, enabled: bool) -> dict[str, Any]:
    return {
        "enabled": enabled,
...
def _combined_event(outputs: Any) -> dict[str, Any] | None:
    if outputs.combined_diagnostics is None:
        return None
    return {
        "draft_event": "R3_4",
