...
def _append_skater_rows(request: _AppendSkaterRowsRequest) -> None:
    for player_id, (player_team, rate, pos) in request.players.items():
        if player_team != request.team:
            continue
        goals = int(request.rng.poisson(max(rate * 0.5, 0.01)))
        assists = int(request.rng.poisson(max(rate * 0.5, 0.01)))
        request.rows.append(
            _fixture_skater_row(
...
def _emit_playoff_series(request: _PlayoffSeriesRequest) -> int:
    ...
    for game_number, (home, away) in enumerate(_PLAYOFF_PAIRINGS):
        ...
        _emit_team_game(
            request.tg_rows,
            _EmitTeamGameInput(gid, date, request.season_id, 3, home, away),
        )
