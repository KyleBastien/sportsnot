...
export function RoundTransitionPage() {
  ...
  const sortedMembers = [
    ...((league?.league_members ?? []) as TransitionMember[]),
  ].sort((a, b) => (a.total_points ?? 0) - (b.total_points ?? 0));
  ...
  const handleStartReDraft = async () => {
    ...
    const reDraftOrder = sortedMembers.map((m) => m.user_id);
        ...
        {completedDrafts && completedDrafts.length > 0 && (
          <Card shadow="sm" padding="lg" radius="md" withBorder>
            <Stack gap="md">
              ...
              {(completedDrafts as CompletedDraft[]).map((d) => (
